...
def calculate_centering_data(box, frame_width, frame_height):
    ...
    diagonal_distance_normalized = diagonal_distance / (
        np.sqrt(frame_width**2 + frame_height**2) / 2)

    center_threshold = 8
    is_centered = (abs(x_offset_percent) < center_threshold
                   and abs(y_offset_percent) < center_threshold)
...
def load_models():
    try:
        ...
        print(
            "Required packages: torch torchvision opencv-python pillow geopy matplotlib"
        )
        exit(1)
...
def get_thread_local_tensor():
    if not hasattr(thread_local, 'tensor'):
        thread_local.tensor = torch.zeros((1, 3, 256, 256),
                                          dtype=torch.float32,
                                          device=device)
...
def triangulate_position(target_depth, target_data):
    try:
        ...
        lon_change = (distance * np.sin(bearing_rad)) / (
            111320 * np.cos(np.radians(camera_lat)))
        ...
        if (tracking.last_gps_update == 0 or
            ...
            tracking.target_gps = (tracking.smoothed_gps[0] +
                                   movement_noise_lat,
                                   tracking.smoothed_gps[1] +
                                   movement_noise_lon)
...
def draw_targeting_interface(frame, target_data=None):
    ...
    if target_data:
        cv2.line(overlay, (FRAME_CENTER_X, FRAME_CENTER_Y),
                 (target_data["center_x"], target_data["center_y"]),
                 COLOR_GREEN, 1)
        ...
        if tracking.locked:
            ...
            thickness = 2
            size = max(30,
                       min(target_data["width"], target_data["height"]) // 2)
            x, y = target_data["center_x"], target_data["center_y"]
            for dx, dy in [(-size, -size), (size, -size), (-size, size),
                           (size, size)]:
                cv2.line(overlay, (x + dx, y + dy),
                         (x + dx + 10 * (1 if dx > 0 else -1), y + dy),
                         box_color, thickness)
                cv2.line(overlay, (x + dx, y + dy),
                         (x + dx, y + dy + 10 * (1 if dy > 0 else -1)),
                         box_color, thickness)
        else:
        ...
        info_texts = [
            "TARGET DETECTED", f"DIST: {tracking.target_distance:.1f}m",
    ...
    cv2.putText(overlay, timestamp, (20, h - 30), cv2.FONT_HERSHEY_SIMPLEX,
                0.6, COLOR_WHITE, 1)
...
def generate_dashboard(tracking_data):
    ...
    time_values = [
        datetime.datetime.fromtimestamp(t) for t in tracking_data.time_history
    ]

    # Ensure all histories are the same length
    min_length = min(len(tracking_data.time_history),
                     len(tracking_data.gps_history),
                     len(tracking_data.distance_history),
                     len(tracking_data.speed_history),
    ...
    if gps_history:
        ...
        def gps_to_relative(lat, lon):
            ...
            lon_diff = (lon - camera_lon) * 111320 * np.cos(
                np.radians(camera_lat))  # meters East
            return lon_diff, lat_diff  # East, North

        rel_x, rel_y = zip(
            *[gps_to_relative(lat, lon) for lat, lon in gps_history])
        ...
        for zone_name, (zone_lat, zone_lon) in LANDMARKS.items():
            ...
            ax_gps.plot(zx,
                        ...
                        marker='o',
                        markersize=10,
                        color=GARUDAKSHAK_ORANGE,
                        label=zone_name if zone_name == "Zone A" else "")

        if tracking_data.target_gps:
            tx, ty = gps_to_relative(tracking_data.target_gps[0],
                                     tracking_data.target_gps[1])
            ax_gps.plot(tx,
                        ty,
                        marker='*',
                        markersize=15,
        ...
        ax_gps.set_xlim(min(rel_x) - x_margin,
                        max(rel_x) + x_margin) if rel_x else (-10, 10)
        ax_gps.set_ylim(min(rel_y) - y_margin,
                        max(rel_y) + y_margin) if rel_y else (-10, 10)

    ax_gps.set_title('GPS Tracking Path',
                     fontsize=14,
                     fontweight='bold',
                     pad=10)
    ...
    if distance_history:
        ax_dist.plot(time_values,
                     distance_history,
                     color='#00cc44',
                     label='Distance (m)')
    ax_dist.set_title('Target Distance',
                      fontsize=14,
                      fontweight='bold',
                      pad=10)
    ...
    if speed_history:
        ...
        smoothed_speed = np.convolve(speed_history,
                                     np.ones(5) / 5,
                                     mode='valid')
        smoothed_time = time_values[len(time_values) - len(smoothed_speed):]
        ax_vel.plot(smoothed_time,
                    smoothed_speed,
                    color='#0088ff',
                    label='Velocity (m/s)')
    ...
    if lock_status_history:
        ax_lock.step(time_values,
                     lock_status_history,
                     where='post',
                     color=GARUDAKSHAK_ORANGE)
        ax_lock.fill_between(time_values,
                             0,
                             lock_status_history,
                             step='post',
                             alpha=0.4,
                             color=GARUDAKSHAK_ORANGE)
    ax_lock.set_title('Target Lock Status',
                      fontsize=14,
                      fontweight='bold',
                      pad=10)
    ...
    summary_data = {
        ...
        'Value': [
            f"{tracking_data.target_distance:.1f}"
            if tracking_data.target_distance else 'N/A',
            f"{speed_history[-1]:.1f}" if speed_history else 'N/A',
            'Locked' if tracking_data.locked else 'Unlocked'
        ]
    }
    table = plt.table(cellText=[summary_data['Value']],
                      colLabels=summary_data['Metrics'],
                      loc='bottom',
                      bbox=[0.75, -0.25, 0.2, 0.15],
                      cellLoc='center')
...
def process_frame(frame, face_detector, midas_model, executor):
    ...
    if num_targets > 0:
        ...
        if w * h >= 1000:
            target_data = calculate_centering_data(box, DISPLAY_WIDTH,
                                                   DISPLAY_HEIGHT)
            ...
            tracking.target_gps = triangulate_position(
                tracking.target_distance, target_data)

            if tracking.last_position and tracking.target_gps:
                elapsed_time = current_time - tracking.last_time
                dist = geodesic(tracking.last_position,
                                tracking.target_gps).meters
            ...
            if target_data["is_centered"]:
                tracking.lock_counter = min(tracking.lock_counter + 1,
                                            TARGET_LOCKED_THRESHOLD)
    ...
    if target_data:
        x_instruction = (
            f"Move {'left' if target_data['x_offset_percent'] < 0 else 'right'} "
            f"{abs(target_data['x_offset_percent'])}%")
        y_instruction = (
            f"Move {'down' if target_data['y_offset_percent'] < 0 else 'up'} "
    ...
    print(
        f"0: {DISPLAY_HEIGHT}x{DISPLAY_WIDTH} {target_text}, {total_time:.1f}ms"
    )
    print(
    ...
    print(
        f"Position: {gps_text}, Adjustment: {movement_text}, Distance: {tracking.target_distance:.1f}m\n"
    )

    tracking.last_time = current_time

    # Store the processed frame for video feed
...
async def send_drone_data():
    ...
    try:
        ...
        print(f"Drone data error: {str(e)}")
        # Try to reconnect after 5 seconds
        await asyncio.sleep(5)
    ...
    with ThreadPoolExecutor(max_workers=2) as executor:
        ...
        try:
            while True:
                ...
                processed_frame = process_frame(frame, face_detector,
                                                midas_model, executor)
